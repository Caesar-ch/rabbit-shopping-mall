    ...
    <dl v-for="item in goods.specs" :key="item.id">
      ...
      <dd>
        <template v-for="val in item.values" :key="val.name">
          <img :class="{ selected: val.selected, disabled: val.disabled }" @click="changeSku(val, item.values)"
            v-if="val.picture" :src="val.picture" :title="val.name">
          <span :class="{ selected: val.selected, disabled: val.disabled }" @click="changeSku(val, item.values)"
...
const getPathMap = (skus) => {
  ...
  skus.forEach(sku => {
    ...
    if (sku.inventory > 0) {
      ...
      const valueArr = sku.specs.map(value => value.valueName) //-valueArr[骨白色，男M（40-41） ]
      // console.log('2222');
      // console.log(valueArr);
      // 可选值数组子集
      const valueArrPower = powerSet(valueArr)//-拿到子集的数组，[ [骨白色，男M（40-41） ]，[骨白色],[男M（40-41）] ]
      // 遍历子集，往pathMap中插入子集，约定key是以 ☆ 进行拼接的，例如：['蓝色', '50cm'] ==> '蓝色☆50cm'
      valueArrPower.forEach(arr => {
        const key = arr.join(spliter)//' 颜色☆尺码 '
...
const getSelectedValues = (specs) => {
  ...
  specs.forEach(item => {
    // 选择的按钮对象
    const selectedVal = item.values.find(val => val.selected)//找到一个选中的
    arr.push(selectedVal ? selectedVal.name : undefined)//push进数组
  })
  return arr
}
...
export default {
  ...
  setup(props, { emit }) {
    ...
    //2.-当点击时，拿着val和item.value.value，val主要用于改变状态，item.value主要用于排他思想
    const changeSku = (val, item) => {
      // 如果当前商品无货，则不能点击
      if (val.disabled) return
      // 取消其他的选择，选中当前的选择
      if (val.selected) {//已经选中则，就取消选中
        val.selected = false
      } else {//没有选中就先将同类别的排他思想
      ...
      // 2. 不是完整的sku组合按钮，提交空对象给父组件       //-拿到所有规格属性的选中的文本信息去查询，进行数据提交和向父组件传值
      const validSelectedValues = getSelectedValues(props.goods.specs)//-拿到了选中的名字数组['黑色','37码']
      // 判断 如果数组中没有undefined，则选中了完整路径
      if (!validSelectedValues.includes(undefined)) {
        // 寻找对应的id
        const skuIds = pathMap[validSelectedValues.join(spliter)]
        // 从当前所有组合中 按照id寻找对应的组合
        const sku = props.goods.skus.find(sku => sku.id === skuIds[0])//-因为当完全选择规格完整时，这个skuIds是一个只有一个值得数组
        emit('change', {//-将这些信息传出去
          skuId: sku.id, // id
          price: sku.price, // 新价格
        ...
        emit('change', {}) //如果规格没选完整就走的这里传出去空对象，外不会根据这个判断为规格未完整
      }
    }
