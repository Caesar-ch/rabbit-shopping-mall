      ...
      <div class="cart">
        <table>
          <thead>
            <tr>
              <th width="120">
                <XtxCheckbox @change="($event) => checkAll($event)" :modelValue="$store.getters['cart/isCheckAll']">全选
                </XtxCheckbox>
              </th>
          ...
          <tbody>
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              <!-- 绑定一个函数，就可以同时拿到当前组件数据和该组件内emit的数据了 -->
              <td>
                <!-- //点击时会将这个skuid和选中的状态发出去 -->
              ...
              <td class="tc">
                ...
                <p v-if="goods.price - goods.nowPrice > 0">比加入时降价 <span class="red">&yen;{{ goods.price -
                    goods.nowPrice
                }}</span></p>
              </td>
              <td class="tc">
              ...
              <td class="tc">
                <p class="f16 red">&yen;{{ Math.round(goods.nowPrice * goods.count).toFixed(2) }}</p>
              </td>
              <td class="tc">
          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/inValidList']" :key="goods.skuId">
              <td>
                <XtxCheckbox @change="($event) => checkOne(goods.skuId, $event)" style="color:#eee;" />
              </td>
              <td>
                <div class="goods">
              ...
              <td class="tc">
                <p>&yen;{{ Math.round(goods.nowPrice * goods.count).toFixed(2) }}</p>
              </td>
              <td class="tc">
                <p><a @click="deleteCart(goods.skuId)" class="green" href="javascript:;">删除</a></p>
                <p><a href="javascript:;">找相似</a></p>
...
export default {
  ...
  setup() {
    ...
    const batchDeleteCart = (isClear) => {
      Confirm({ text: `亲，是否确认删除${isClear ? '失效' : '选中'}的商品？` })
        .then(res => {
    ...
    const checkout = () => {
      ...
      // 2. 如果登录了就直接跳转到结算页
      if (store.state.user.profile.token) return router.push('/member/checkout')
      // 3. 未登录的弹出确认框，提示：下单结算需要登录
      Confirm({ text: '下单结算需要登陆，现在去登录吗？' })
        .then(() => { router.push('/member/checkout') })
        .catch(e => { })
    }
    return { checkOne, checkAll, deleteCart, updateCount, batchDeleteCart, updateCartSku, checkout }
  }
}
</script>
