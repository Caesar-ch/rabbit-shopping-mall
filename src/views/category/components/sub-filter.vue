<template>
  ...
  <div class="sub-filter" v-if="filterData && !filterLoading">
    ...
    <div class="item">
      ...
      <div class="body">
        <a @click="changeBrand(i.id)" :class="{ active: i.id === filterData.selectedBrand }" href="javascript:;"
    ...
    <div class="item" v-for="i in filterData.saleProperties" :key="i.id">
      ...
      <div class="body">
        <a @click="changeProp(i, prop.id)" :class="{ active: prop.id === i.selectedProp }" href="javascript:;"
          v-for="prop in i.properties" :key="prop.id">{{ prop.name }}</a>
      </div>
    </div>
  </div>
  ...
  setup(props, { emit }) {
    ...
    const getFilterParams = () => {
      ...
      filterData.value.saleProperties.forEach(item => {
        ...
        }//将找到的这一项properties.name 和这一项所属的saleProperties.name 传出去,接口会识别这个字段请求新数据
      })
